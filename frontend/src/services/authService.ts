...
export async function verifyRegister(payload: VerifyRequest): Promise<VerifyResponse> {
    try {
        const response = await api.post<VerifyResponse>("/auth/verify_code/", payload, {withCredentials: true});
...
export async function verifyLogin(payload: VerifyRequest): Promise<VerifyResponse> {
    try {
        const response = await api.post<VerifyResponse>("/auth/login/", payload, {withCredentials: true});
...
export async function verifyOTP(payload: VerifyRequest): Promise<VerifyResponse> {
    try {
        const response = await api.post<VerifyResponse>("/auth/verify_code/", payload, {withCredentials: true});
        if (response.data.token) {
            localStorage.setItem("auth_token", response.data.token);
