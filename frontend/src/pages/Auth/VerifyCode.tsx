...
import { verifyRegister, verifyLogin, resendVerificationCode } from '@/services/authService';
...
const VerifyCode = () => {
  ...
  useEffect(() => {
    if (!phoneNumber || !username) {
      setError('Telefon raqami yoki username topilmadi. Iltimos, qayta ro‘yxatdan o‘ting.');
  ...
  }, [phoneNumber, username, navigate, resendCooldown]);
  ...
  const verifyCodeHandler = async () => {
    ...
    try {
      ...
      const response = location.pathname.includes('/verify-login')
        ? await verifyLogin({ phone_number: phoneNumber, code: verificationCode })
        : await verifyRegister({ phone_number: phoneNumber, code: verificationCode });
...
const resendCode = async () => {
  ...
  setError('');
  try {
    const action = location.pathname.includes('/verify-login') ? 'login' : 'register';
      ...
      <div className="relative w-full max-w-md mx-auto p-4">
        <div className="bg-white backdrop-blur-xl rounded-3xl border border-gray-700/50 shadow-2xl overflow-hidden">
          ...
          <div className="px-6 pb-6">
            ...
            {error && (
              <div className="mb-4 text-red-600 text-sm bg-red-50 border border-red-200 rounded-lg p-3 text-center">
                {error}
              </div>
            )}
